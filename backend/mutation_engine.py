...
if __name__ == "__main__":
    ...
    print("\n--- Accuracy Calculation (OLD - all letters) ---")
    original = "ninja taco"
    test_cases = [
    ...
    for typed, desc in test_cases:
        ...
        print(f"  '{typed}' → {acc}% ({desc})")

    print("\n--- Accuracy Calculation (NEW - only blanks) ---")
    # Simulate: original is "ninja", player saw "n_n_a", typed "ninja"
    original = "ninja"
    typed = "ninja"
    # Positions 1 and 3 were blanks (i, j)
    blank_positions = {1, 3}
    
    acc_all = calculate_accuracy(original, typed)
    acc_blanks = calculate_accuracy(original, typed, blank_positions)
    print(f"  Original: '{original}'")
    print(f"  Saw:      'n_n_a' (blanks at positions 1, 3)")
    print(f"  Typed:    '{typed}'")
    print(f"  Old method (all letters): {acc_all}%")
    print(f"  New method (only blanks): {acc_blanks}%")
    
    # Now test with wrong input
    typed_wrong = "nxnxa"
    acc_blanks_wrong = calculate_accuracy(original, typed_wrong, blank_positions)
    print(f"\n  Typed wrong: '{typed_wrong}'")
    print(f"  Accuracy (only blanks): {acc_blanks_wrong}%")
